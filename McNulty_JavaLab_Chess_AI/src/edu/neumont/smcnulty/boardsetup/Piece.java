...
public enum Piece 
{
    ...
    ********************/ 
	...
	public ArrayList<String> getValidMoves(String start, HashMap<String, Piece> pieceMap)
	{
	...
	public static boolean inStalemate(HashMap<String, Piece> pieceMap)
	{
		boolean someBodyInStaleMate = false;
		//take the pieceMap, call findAllPossibleMoves()
	    ArrayList<Move> moves = AI.findAllPossibleMoves(true, pieceMap);
		
	    if(moves.size() == 0 && !Piece.inCheckMate(true, pieceMap))
	    {
	    	someBodyInStaleMate = true;
	    }
	    else if(moves.size() == 0 && !Piece.inCheckMate(false, pieceMap))
	    {
	    	someBodyInStaleMate = false;
	    }
	    	
			return someBodyInStaleMate;
	}
}
