...
public class Game extends MouseAdapter
{
	...
	public Game()
	{
		...
		//pass in true for light, false for dark when making a new AI.
		chester = new AI(false);
		betty = new BetterAI(true);
	...
	public void mousePressed(MouseEvent e)
	{
		...
		else if(selection != null)
		{
				...
				if(validMoves.contains(click2.getLoc()))
				{
					...
					handleCheckAndCheckMate(false);
					
					if(!gameOver)
					{
						makeAMove(lightTurn, chester.aiGetTurn(model.getPieceMap()));
					}
					
					lightTurn = !lightTurn;
					
					handleCheckAndCheckMate(true);
	...
	private void handleCheckAndCheckMate(boolean showDialogBox) 
	{
	...
	public void runMovesFile()
	{
		...
		for(String s : list)
		{
			...
			currentTurnIsLight = makeAMove(currentTurnIsLight, s);
		}		
	}



	public boolean makeAMove(boolean currentTurnIsLight, String s) 
	{
		...
		return currentTurnIsLight;
	}


	// game flow when used for two AI players
	public void gameFlow() 
	{
		//set up board, pieces
		setPieces();
		
		AI currentPlayer = betty;
		
		while(!gameOver)
		{

			makeAMove(lightTurn, currentPlayer.aiGetTurn(model.getPieceMap()));
			
			lightTurn = !lightTurn;
			
			handleCheckAndCheckMate(false);

			if(currentPlayer == betty)
			{
				currentPlayer = chester;
			}
			else
			{
				currentPlayer = betty;
